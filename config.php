...
foreach($CONF as $key=>$value) {
    if (array_key_exists($key,$validate)) {
        switch($validate[$key]) {
            case "int": if (!preg_match("/^[0-9]*$/i",$value)) echo PMBP_addOutput(array_search($key,$conf)."' ".C_WRONG_TYPE,"red");
                break;
            case "float": if (!preg_match("/^[0-9]*\.?[0-9]*$/i",$value)) echo PMBP_addOutput(array_search($key,$conf)."' ".C_WRONG_TYPE,"red");
                break;
