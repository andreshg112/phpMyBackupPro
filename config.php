...
if (isset($_POST['submit'])) {
    ...
    if (isset($_POST['sql_host']) || isset($_POST['del_time'])) {
        ...
        foreach ($checkbox as $item => $page) {
            ...
            if ($page == "2" && isset($_GET['ext'])) {
                $CONF[$item] = "0";
            }

            // checkboxes on basic configurations
            elseif ($page == "1" && !isset($_GET['ext'])) {
                $CONF[$item] = "0";
            }
        ...
        foreach ($CONF as $item => $value) {
            ...
            if ($_SESSION['multi_user_mode']) {
                if ($item == "sitename" && $PMBP_MU_CONF['sitename']) {
                    continue;
                }

                if (($item == "email_use" | $item == "email") && !$PMBP_MU_CONF['allow_email']) {
                    continue;
                }
...
foreach ($CONF as $key => $value) {
    if (array_key_exists($key, $validate)) {
        switch ($validate[$key]) {
            case "int":if (!preg_match("/^[0-9]*$/i", $value)) {
                    echo PMBP_addOutput(array_search($key, $conf) . "' " . C_WRONG_TYPE, "red");
                }

                break;
            case "float":if (!preg_match("/^[0-9]*\.?[0-9]*$/i", $value)) {
                    echo PMBP_addOutput(array_search($key, $conf) . "' " . C_WRONG_TYPE, "red");
                }
...
if (isset($_SESSION['sql_host_org'])) {
    ...
    if ($_SESSION['sql_db_org']) {
        if (!@mysqli_select_db($con, $_SESSION['sql_db_org'])) {
...
// print html code
if (isset($_GET['ext'])) {
    echo "<br><form action=\"config.php?ext=TRUE\" method=\"post\">\n";
} elseif (isset($_GET['sys'])) {
    echo "<br><form action=\"config.php?sys=TRUE\" method=\"post\">\n";
} else {
    echo "<br><form action=\"config.php\" method=\"post\">\n";
}
...
function PMBP_config_print($item)
{
    ...
    if (isset($checkbox[$item])) {
        ...
        foreach ($select[$item] as $opt_no => $print_value) {
            if ($opt_no == $confItem) {
                $out .= "<option value=\"" . $opt_no . "\" selected>" . $print_value . "</option>\n";
            } else {
                $out .= "<option value=\"" . $opt_no . "\">" . $print_value . "</option>\n";
            }
        }
