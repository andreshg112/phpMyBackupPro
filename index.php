...
if (!is_writeable(PMBP_GLOBAL_CONF)) {
    echo PMBP_addOutput(I_CONF_ERROR, "red_left");
}

// if export directory is not writeable
if (!is_writeable(PMBP_EXPORT_DIR)) {
    echo PMBP_addOutput(I_DIR_ERROR, "red_left");
}

// if first use or no db-connection possible
if (!PMBP_mysql_connect()) {
    echo PMBP_addOutput(C_WRONG_SQL, "red_left");
}

if ($CONF['sql_db'] && !@mysqli_select_db($CONF['sql_db'])) {
    echo PMBP_addOutput(C_WRONG_DB, "red_left");
}
...
if (function_exists("phpversion")) {
    ...
    if ($phpvers >= 4.3) {
        ...
        $php = PMBP_addOutput($tmp, "red_left");
    }
...
if (!function_exists("mysqli_get_server_info")) {
    ...
    $mysql_s = @mysqli_get_server_info($con);
}

if (!function_exists("mysqli_get_client_info")) {
    $mysql_c = PMBP_I_NO_RES;
} else {
    $mysql_c = @mysqli_get_client_info();
}
