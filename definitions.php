...
if (isset($override_user_date)) {
    ...
    if ($PMBP_MU_CONF['sitename']) {
        $CONF['sitename'] = $PMBP_MU_CONF['sitename'];
    }
...
if (count($CONF['sql_host_s'])) {
    ...
    if (!isset($_SESSION['sql_host_org']) || !$_SESSION['LOGGED_IN']) {
        $_SESSION['sql_host_org'] = $CONF['sql_host'];
    }

    if (!isset($_SESSION['sql_user_org']) || !$_SESSION['LOGGED_IN']) {
        $_SESSION['sql_user_org'] = $CONF['sql_user'];
    }

    if (!isset($_SESSION['sql_passwd_org']) || !$_SESSION['LOGGED_IN']) {
        $_SESSION['sql_passwd_org'] = $CONF['sql_passwd'];
    }

    if (!isset($_SESSION['sql_db_org']) || !$_SESSION['LOGGED_IN']) {
        $_SESSION['sql_db_org'] = $CONF['sql_db'];
    }

    if (!isset($_SESSION['wss']) || !$_SESSION['LOGGED_IN']) {
        $_SESSION['wss'] = -1;
    }

    if (isset($_POST['mysql_host'])) {
        $_SESSION['wss'] = $_POST['mysql_host'];
    }
